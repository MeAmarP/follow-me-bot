...
class BYTETracker(object):
    def __init__(self,frame_rate=30):
        ...
        self.frame_id = 0
        self.det_thresh = 0.4 + 0.1 #tracker detection threshold
        self.buffer_size = int(frame_rate / 30.0 * 20) #tracker_buffer_size
    ...
    def update(self, dets):
        ...
        remain_indexes = dets[:, 4] > 0.5 #self.opt.conf_thres
        indexes_low = dets[:, 4] > 0.2
        indexes_high = dets[:, 4] < 0.5 #self.opt.conf_thres
        indexes_second = np.logical_and(indexes_low, indexes_high)
        dets_second = dets[indexes_second]
        dets = dets[remain_indexes]
        ...
        matches, u_track, u_detection_second = matching.linear_assignment(dists, thresh=0.4)# match_iou_threshold=0.4
        for itracked, idet in matches:
